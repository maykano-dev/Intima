...
const statusConfig: Record<string, { label: string; color: string; icon: string }> = {
  pending: { label: 'Order Placed', color: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300', icon: '🕐' },
  paid: { label: 'Payment Confirmed', color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300', icon: '✓' },
  processing: { label: 'Processing', color: 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300', icon: '⟳' },
  shipped: { label: 'Shipped', color: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300', icon: '📦' },
...
export default function OrderDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      <div className="mb-8">
        <Link href="/track" className="text-sm text-primary hover:underline inline-flex items-center gap-1 mb-4">
            ...
            return (
              <div key={s} className="flex items-start gap-4 pb-6 last:pb-0 relative">
                ...
                <div className={cn(
                  ...
                  {isCurrent ? cfg.icon : isComplete ? '✓' : i + 1}
                </div>
                <div className="pt-1">
                  <p className={cn('font-medium text-sm', isCurrent ? 'text-foreground' : isComplete ? 'text-muted' : 'text-muted/50')}>
                    {cfg.label}
                  </p>
                  {isCurrent && (
      ...
      <div className="rounded-2xl border border-border p-6 mb-6">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">Discreet Packaging</span>
          </div>
        </div>
      </div>
